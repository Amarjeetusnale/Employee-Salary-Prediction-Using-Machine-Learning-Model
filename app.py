...
with st.container():
    ...
    with st.expander("📘 What does Educational-num mean?"):
        st.markdown("""
...
marital_encoder.classes_ = np.array(['Divorced', 'Married-AF-spouse', 'Married-civ-spouse', 'Married-spouse-absent', 'Never-married', 'Separated', 'Widowed'])
...
input_df['marital-status'] = marital_encoder.transform([input_df['marital-status'][0]])
...
if st.button("🔍 Predict Salary Category"):
    ...
    if hasattr(model, "predict_proba"):
        ...
        <div class="info-box">
            <ul>
                <li>💲 <b>Probability for class 0 (≤50K)</b>: <span>{proba[0]*100:.2f}%</span></li>
                <li>💰 <b>Probability for class 1 (>50K)</b>: <span>{proba[1]*100:.2f}%</span></li>
            </ul>
            <p><i>The probabilities above show how likely the model thinks the input belongs to each class.</i></p>
        </div>
        """, unsafe_allow_html=True)
